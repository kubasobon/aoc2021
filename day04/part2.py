...
if __name__ == "__main__":
    ...
    boards_that_won = []
    number_idx = 0
    while len(boards_that_won) < len(boards) and number_idx < len(numbers):
        n = numbers[number_idx]
        for b in boards:
            if b in boards_that_won:
                continue
            b.call_out(n)
            if b.has_bingo():
    ...
    if len(boards_that_won) < len(boards):
        print("E! could not determine last winning board")
        sys.exit(1)
    else:
        b = boards_that_won[-1]
        n = numbers[number_idx - 1]
        print(f"num: {n}")
